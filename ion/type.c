...
bool is_signed_type(Type* type)
{
    switch (type->kind)
    {
        // TODO(nicholas): TYPE_CHAR signedness is platform independent, needs to factor into backend.
        case TYPE_SCHAR:
        case TYPE_SHORT:
        case TYPE_INT:
        case TYPE_LONG:
...
void type_complete_struct(Type* type, TypeField* fields, size_t num_fields)
{
    ...
    for (TypeField* it = fields; it != fields + num_fields; ++it)
    {
        ...
        type->align = MAX(type->align, type->alignof(it->type));
